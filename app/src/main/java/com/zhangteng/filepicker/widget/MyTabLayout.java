...
import com.zhangteng.filepicker.utils.TextUtil;
...
public class MyTabLayout extends HorizontalScrollView {
    ...
    public MyTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mTabTextSize = a.getDimensionPixelSize(R.styleable.MyTabLayout_tabMyTextSize, (int) mTabTextSize);
        mTabSelectedTextSize = a.getDimensionPixelSize(R.styleable.MyTabLayout_tabMySelectedTextSize, (int) mTabTextSize);
        ...
        int tabViewNumber = a.getInt(R.styleable.MyTabLayout_tabMyViewNumber, -1);
        if (tabViewNumber != -1) {
    ...
    void selectTab(final MyTabLayout.Tab tab, boolean updateIndicator) {
        ...
        if (currentTab == tab) {
            ...
            if (currentTab != null) {
                TextView currentTabTextView = currentTab.mView.mTextView;
                if (mTabTypeface != mTabSelectedTypeface && currentTabTextView.getTypeface().getStyle() != mTabTypeface) {
                    currentTabTextView.setTypeface(Typeface.defaultFromStyle(mTabTypeface));
                }
                if (mTabTextSize != mTabSelectedTextSize && currentTabTextView.getTextSize() != mTabTextSize) {
                    currentTabTextView.setTextSize(mTabTextSize);
                }
                dispatchTabUnselected(currentTab);
            }
            mSelectedTab = tab;
            if (tab != null) {
                TextView tabTextView = tab.mView.mTextView;
                if (mTabTypeface != mTabSelectedTypeface && tabTextView.getTypeface().getStyle() != mTabSelectedTypeface) {
                    tabTextView.setTypeface(Typeface.defaultFromStyle(mTabSelectedTypeface));
                }
                if (mTabTextSize != mTabSelectedTextSize && tabTextView.getTextSize() != mTabSelectedTextSize) {
                    tabTextView.setTextSize(mTabSelectedTextSize);
                }
    ...
    class TabView extends LinearLayout {
        ...
        public void onMeasure(final int origWidthMeasureSpec, final int origHeightMeasureSpec) {
            ...
            if (mTextView != null) {
                ...
                float selectedTextSize = mTabSelectedTextSize;
                int maxLines = mDefaultMaxLines;

                if (mTabSelectedTypeface != mTabTypeface) {
                    mTextView.setTypeface(Typeface.defaultFromStyle(mTextView.isSelected() ? mTabSelectedTypeface : mTabTypeface));
                } else if (mTextView.getTypeface().getStyle() != mTabTypeface) {
                ...
                if (textSize != curTextSize || selectedTextSize != curTextSize || (curMaxLines >= 0 && maxLines != curMaxLines)) {
                    ...
                    if (updateTextView) {
                        mTextView.setTextSize(TypedValue.COMPLEX_UNIT_PX, mTextView.isSelected() ? selectedTextSize : textSize);
