...
public class VideoPickerConfig {
    ...
    private String filePath;            // 拍照以及截图后 存放的位置。    默认：/videoPicker/Pictures
    private ArrayList<String> pathList;      // 已选择照片的路径
    private boolean isOpenCamera;             // 是否直接开启相机    默认：false
    private Builder builder;

    public VideoPickerConfig(Builder builder) {
        setBuilder(builder);
    }

    public static VideoPickerConfig getInstance() {
        if (videoPickerConfig == null) {
            synchronized (VideoPickerConfig.class) {
                if (videoPickerConfig == null) {
    ...
    private void setBuilder(Builder builder) {
        ...
        VideoPickerConfig.videoPickerConfig = this;
    }
    ...
    public static class Builder implements Serializable {
        ...
        private String filePath = "/videoPicker/videoPickerPictures";
        ...
        public VideoPickerConfig build() {
            if (videoPickerConfig == null) {
                videoPickerConfig = new VideoPickerConfig(this);
            } else {
                videoPickerConfig.setBuilder(this);
            }
            return videoPickerConfig;
        }
